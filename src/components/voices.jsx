...
const Voices = () => {
    ...
    return (
        <section className="w-full bg-[#f3f4f6] py-20 px-4 sm:px-6 lg:px-8">
            <div className="max-w-[1150px] mx-auto">
                {/* heading */}
                <div className="text-center max-w-[820px] mx-auto">
                    <h2 className="text-[#111111] text-[60px] sm:text-[30px] lg:text-[40px] font-bold">
                        Voice of the Diaspora
                    </h2>

                    <p className="mt-6 text-[#2f2f2f] text-[10px] sm:text-[14px] leading-[1.45] max-w-[650px] font-bold mx-auto">
                ...
                <div className="mt-16 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6  ">
                    {testimonials.map((item, index) => (
                        <div key={index} className="flex flex-col justify-between bg-white   rounded-[20px] ">
                            <div className=' mb-4' >
                                <img src="shigo.png" alt="logo" className=' ' />

                            </div>
                            {/* quote */}
                            <div className="px-2">
                                <p className="text-[#111111] italic text-[19px] sm:text-[20px]">
                                    “{item.quote}”
                                </p>
                            </div>

                            {/* profile strip */}
                            <div className="mt-10 rounded-[10px] bg-[#FFB700] px-4 py-4 flex items-center gap-4 min-h-[84px]">
                                <img
                                    src={item.avatar}
                                    alt={item.name}
